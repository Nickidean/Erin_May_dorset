...
export async function handler(event) {
  ...
  if (existing.length >= MAX_GALLERY_IMAGES) {
    return { statusCode: 400, body: JSON.stringify({ error: `Maximum ${MAX_GALLERY_IMAGES} images allowed` }) }
  }
  ...
  await logActivity(session.adminName, 'add_gallery_image', { storagePath: path })
